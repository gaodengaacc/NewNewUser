...
public class AddressDictManager {
    ...
    public List<Province> getProvinceList(){
        ...
        Cursor cursor = db.rawQuery("select * from " + TableField.TABLE_ADDRESS_DICT+" where "+ ADDRESS_DICT_FIELD_PARENTID+"=?"+" order by cast(code as int)", new String[]{String.valueOf(0)});
        while (cursor.moveToNext()){
    ...
    public List<City> getCityList(int  provinceId){
        ...
        Cursor cursor = db.rawQuery("select * from " + TableField.TABLE_ADDRESS_DICT+" where "+ ADDRESS_DICT_FIELD_PARENTID+"=?"+" order by cast(code as int)", new String[]{String.valueOf(provinceId)});
        while (cursor.moveToNext()){
            City city =  new City();
    ...
    public List<Street> getStreetList(int countyId){
        ...
        Cursor cursor = db.rawQuery("select * from " + TableField.TABLE_ADDRESS_DICT+" where "+ ADDRESS_DICT_FIELD_PARENTID+"=?"+" order by cast(code as int)", new String[]{String.valueOf(countyId)});
        while (cursor.moveToNext()){
